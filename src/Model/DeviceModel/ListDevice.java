...
public void addDevice(String name, String idType, double price) {
    ...
    try {
        ...
        String sql = "INSERT INTO device (id_device, name_device, id_devicetype, price) VALUES (?, ?, ?, ?)";
    ...
    try {
        ...
        // Câu lệnh SQL để xóa thiết bị dựa vào `id_device`
        String sql = "DELETE FROM device WHERE id_device = ?";
        PreparedStatement stmt = conn.prepareStatement(sql);
        
        // Thiết lập giá trị cho câu lệnh xóa
...
public void updateDevice(String idDevice, String newName, String newIdType, double newPrice) {
    try {
        ...
        // Câu lệnh SQL để cập nhật thông tin thiết bị dựa vào `id_device`
        String sql = "UPDATE device SET name_device = ?, id_devicetype = ?, price = ? WHERE id_device = ?";
        PreparedStatement stmt = conn.prepareStatement(sql);
        
        // Thiết lập giá trị cho câu lệnh cập nhật
        stmt.setString(1, newName);
        stmt.setString(2, newIdType);  // Giả sử newIdType là khóa ngoại hợp lệ trong bảng Device_Type
        stmt.setDouble(3, newPrice);
        stmt.setString(4, idDevice);
...
// Tìm kiếm thiết bị theo ID từ cơ sở dữ liệu
